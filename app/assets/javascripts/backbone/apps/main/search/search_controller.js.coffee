@Equitabletod.module "MainApp.Search", (Search, App, Backbone, Marionette, $, _) ->
	Search.Controller =
	  	...
	  	showSimpleSearchForm: ->
            ...
            gon.vmts = [0 , 14.9, 25.84, 39.8, 124.23] # _.map features, (key, value) -> key.properties.ov_vmthday
            ...
            gon.vehphhs = [0, 0.6475, 1.03, 1.39, 4.51] # _.map features, (key, value) -> key.properties.ov_vehphh
            gon.trnpcmis = [0.022, 1.25, 3.83, 8.97, 28.5] # _.map features, (key, value) -> key.properties.ov_trnpcmi if key.properties.ov_trnpcmi
            gon.ghgs = [0, 15.65, 34, 52.77, 115] # _.map features, (key, value) -> key.properties.ov_ghg if key.properties.ov_ghg
            ...
            gon.hupipes = [0, 0, 113, 676, 8509] # _.map features, (key, value) -> key.properties.ov_hupipe if key.properties.ov_hupipe
            gon.emppipes = [0, 0, 0, 556, 19049] # _.map features, (key, value) -> key.properties.ov_emppipe if key.properties.ov_emppipe
